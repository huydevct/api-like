...
func (adapters Rabbits) Get(name string) (result *Rabbit) {
	if adapter, ok := adapters[name]; ok {
		...
		panic("Không tìm thấy config RabbitV2 " + name)
	}
	return
}
...
type Rabbit struct {
	...
	Consumer RbConsumer `mapstructure:"consumer"`
	// Init thêm
	Err     chan error
...
func (config *Rabbit) Init() {
	...
	// Listen to NotifyClose and reconnect
	go func() {
...
// 1. Declare Exchange, Queue
// 2. Bind queue to Exchange
func (config *Rabbit) PrepareByConsumer(consumer RbConsumer) (err error) {
	// Validate
	{
		if config.conn == nil {
			err = fmt.Errorf("[%s][%s] Chưa init Rabbit", config.Name, config.Host)
...
func (config *Rabbit) Publish(consumer *RbConsumer, data interface{}) error {
...
func (config RbConsumer) getAgrs() amqp.Table {
