	...
	once.Do(func() {
		...
		collection := instance.Session.GetCollection(instance.Collection)
		{
			...
			indexKey := mongo.IndexModel{
				Keys:    bsonx.Doc{{Key: "token", Value: bsonx.Int32(1)}, {Key: "status", Value: bsonx.Int32(1)}},
				Options: indexOpts,
			}
		...
		}
		{
			...
			indexKey := mongo.IndexModel{
				Keys:    bsonx.Doc{{Key: "user_id", Value: bsonx.Int32(1)}, {Key: "status", Value: bsonx.Int32(1)}},
				Options: indexOpts,
			}
			_, err := collection.Indexes().CreateOne(ctx, indexKey)
		...
		}
		{
			...
			indexKey := mongo.IndexModel{
				Keys:    bsonx.Doc{{Key: "user_id", Value: bsonx.Int32(1)}, {Key: "device_id", Value: bsonx.Int32(1)}},
				Options: indexOpts,
			}
			_, err := collection.Indexes().CreateOne(ctx, indexKey)
