	...
	once.Do(func() {
		...
		collection := instance.Session.GetCollection(instance.Collection)
		{
			...
			indexKey := mongo.IndexModel{
				Keys:    bsonx.Doc{{Key: "group_id", Value: bsonx.Int32(1)}, {Key: "_id", Value: bsonx.Int32(1)}},
				Options: indexOpts,
			}
		...
		}
		{
			...
			indexKey := mongo.IndexModel{
				Keys:    bsonx.Doc{{Key: "get_time_random", Value: bsonx.Int32(1)}},
				Options: indexOpts,
			}
			_, err := collection.Indexes().CreateOne(ctx, indexKey)
